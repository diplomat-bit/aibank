...
class BankingProductEligibility(object):
    ...
    def eligibility_type(self, eligibility_type):
        ...
        if eligibility_type is None:
            raise ValueError(
                "Invalid value for `eligibility_type`, must not be `None`"
            )  # noqa: E501
        allowed_values = [
            "BUSINESS",
            "PENSION_RECIPIENT",
        ...
        if eligibility_type not in allowed_values:
            raise ValueError(
                "Invalid value for `eligibility_type` ({0}), must be one of {1}".format(  # noqa: E501
                    eligibility_type, allowed_values
                )
            )
