...
class TestStatus:
    ...
    def test_streaming_response_retrieve(self, client: Jocall3) -> None:
        with client.system.status.with_streaming_response.retrieve() as response:
...
class TestAsyncStatus:
    ...
    async def test_method_retrieve(self, async_client: AsyncJocall3) -> None:
        status = await async_client.system.status.retrieve()
        assert_matches_type(StatusRetrieveResponse, status, path=["response"])

    @parametrize
    async def test_raw_response_retrieve(self, async_client: AsyncJocall3) -> None:
        response = await async_client.system.status.with_raw_response.retrieve()
    ...
    async def test_streaming_response_retrieve(self, async_client: AsyncJocall3) -> None:
        async with async_client.system.status.with_streaming_response.retrieve() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
