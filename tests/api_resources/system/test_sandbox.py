...
class TestSandbox:
    ...
    @parametrize
    def test_method_force_error(self, client: Jocall3) -> None:
        sandbox = client.system.sandbox.force_error(
            error_code=500,
        )
        assert_matches_type(SandboxForceErrorResponse, sandbox, path=["response"])

    @parametrize
    def test_raw_response_force_error(self, client: Jocall3) -> None:
        response = client.system.sandbox.with_raw_response.force_error(
            error_code=500,
        )
    ...
    @parametrize
    def test_streaming_response_force_error(self, client: Jocall3) -> None:
        with client.system.sandbox.with_streaming_response.force_error(
            error_code=500,
        ) as response:
...
class TestAsyncSandbox:
    ...
    @parametrize
    async def test_method_force_error(self, async_client: AsyncJocall3) -> None:
        sandbox = await async_client.system.sandbox.force_error(
            error_code=500,
        )
        assert_matches_type(SandboxForceErrorResponse, sandbox, path=["response"])

    @parametrize
    async def test_raw_response_force_error(self, async_client: AsyncJocall3) -> None:
        response = await async_client.system.sandbox.with_raw_response.force_error(
            error_code=500,
        )
    ...
    @parametrize
    async def test_streaming_response_force_error(self, async_client: AsyncJocall3) -> None:
        async with async_client.system.sandbox.with_streaming_response.force_error(
            error_code=500,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
