...
class BankingProductDepositRate(object):
    ...
    def deposit_rate_type(self, deposit_rate_type):
        ...
        if deposit_rate_type is None:
            raise ValueError(
                "Invalid value for `deposit_rate_type`, must not be `None`"
            )  # noqa: E501
        allowed_values = [
            "FIXED",
            "BONUS",
        ...
        if deposit_rate_type not in allowed_values:
            raise ValueError(
                "Invalid value for `deposit_rate_type` ({0}), must be one of {1}".format(  # noqa: E501
                    deposit_rate_type, allowed_values
                )
            )
