...
finetuned_bert_model = BertForSequenceClassification.from_pretrained(
...
with ThreadPoolExecutor() as executor:
    for i, batch in enumerate(combined_dataloader):
        ...
        print(
            f"Batch {i+1}/{len(combined_dataloader)} completed. Loss: {loss.result()}. Estimated time remaining: {remaining_time:.2f}s"
        )

# Save the retrained model and other necessary files
print("Saving files...")
torch.save(
    finetuned_bert_model.state_dict(), "/content/drive/MyDrive/000/retrained_model.pt"
)
...
with zipfile.ZipFile(
    "/content/drive/MyDrive/000/retrained_model_files.zip", "w"
) as zipf:
...
print(
    f"All files zipped and saved at /content/drive/MyDrive/000/retrained_model_files.zip"
)
